...
## 市场分析驱动增强需求（Epic 9-11）

> 基于市场竞争力分析，识别出的关键功能缺失。按优先级分为P0（核心竞争壁垒）、P1（用户粘性）、P2（商业增长）三级。

### Epic 9: 分配生/指标生政策引擎（P0 - 核心竞争壁垒）

**Epic Goal:** 精准模拟分配生/指标生政策对录取概率的影响，解决家长最大的择校焦虑

**FRs Covered:** FR40-FR44

**FR40:** 系统应支持按学校、按初中、按年度管理分配生/指标生名额数据
**FR41:** 系统应根据学生所在初中和分配名额，自动修正录取概率
**FR42:** 管理员应能够配置不同区县的分配生政策规则
**FR43:** 用户应能够查看目标学校的分配生名额分配情况
**FR44:** 系统应在推荐结果中标注"分配生优势"/"统招竞争"标签

### Epic 10: 志愿方案决策支持（P0 - 核心竞争壁垒）

**Epic Goal:** 提供志愿方案对比、风险评估和决策报告，从数据展示升级为决策支持

**FRs Covered:** FR45-FR52

**FR45:** 用户应能够并排对比两个或多个志愿方案
**FR46:** 系统应提供方案对比维度：录取概率总和、风险分布、保底安全度、冲刺空间
**FR47:** 系统应生成方案整体风险等级（高风险/中风险/低风险）
**FR48:** 系统应计算"滑档概率"（所有志愿都未录取的概率）
**FR49:** 系统应提供智能调整建议（如"建议将XX学校从第3志愿调到第2志愿"）
**FR50:** 用户应能够导出志愿方案评估报告（PDF/图片）
**FR51:** 系统应支持分数到位次的自动换算（基于历史数据）
**FR52:** 系统应展示预测结果的置信区间（如"录取概率 75% ± 8%"）

### Epic 11: 数据可视化与体验增强（P1/P2 - 用户粘性与增长）

**Epic Goal:** 通过可视化图表、学校维度扩充和社交分享提升用户体验和传播

**FRs Covered:** FR53-FR61

**FR53:** 系统应展示历年分数线趋势图（折线图）
**FR54:** 系统应展示录取概率仪表盘图
**FR55:** 系统应展示志愿方案对比雷达图
**FR56:** 学校信息应扩充字段：学校等级、高考升学率、住宿条件、学费、特色班等
**FR57:** 用户应能够分享志愿方案（生成图片/小程序卡片）
**FR58:** 系统应提供新用户3步引导流程（录入成绩→查看推荐→创建方案）
**FR59:** 系统应在关键时间节点发送提醒通知（志愿填报截止日等）
**FR60:** 预测算法应采用多因素回归模型，支持至少5年历史数据
**FR61:** 系统应支持微信模板消息推送和订阅消息
